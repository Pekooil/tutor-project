...
Calyxa is a **TypeScript-end-to-end, Supabase-centric system**. A lightweight **WXT + Preact +
...
// manifest (generated by WXT) — illustrative
{
  ...
  "action": { "default_title": "Calyxa" },
  "commands": {
    "activate-tutor": {
      "suggested_key": { "default": "Ctrl+Shift+M", "mac": "Command+Shift+M" },
      "description": "Open Calyxa on the current page"
    }
  },
...
- **How it avoids mutation:** the content script **never writes to the host DOM**. It only reads.
  The single node it *attaches* is a `<calyxa-root>` host element whose contents live entirely
...
- **Chosen: injected Shadow DOM overlay** (closed shadow root on a single `<calyxa-root>` host,
...
```text
You are Calyxa, a patient, encouraging math tutor for an independent high-school or
...
- **Render & cleanup:** annotations draw on the transparent SVG overlay layer in the shadow root,
  ...
  session end** when `<calyxa-root>` is removed — leaving the host page byte-for-byte unchanged.
...
- **Acceptance criteria:** (1) On 5 representative math sites (KaTeX, MathJax, MathML, plain-text,
  ...
  DOM-diff snapshot proves **zero host-DOM mutation** (only `<calyxa-root>` added). (3) Overlay
