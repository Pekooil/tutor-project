...
// /studio-preview (Notebook Studio) is a self-contained design harness for the
// four-view studio (dashboard · notes · quiz · flashcards): seeded mock data
// only, NO user data, viewable without a session so the design can be reviewed
// and screenshotted inside the real shell (the sibling of the extension's
// pill-harness, and the successor to the retired /notebook-preview). Public like
// '/'; safe to drop before GA.
// /pricing (Landing v6, 2026-07-24) is the plans page the landing's nav and
// footer link to — the landing itself no longer renders a pricing section, so
// this is where a signed-out visitor comparing plans lands. Public like '/'.
const PUBLIC_PATHS = [
  '/',
  '/login',
  '/signup',
